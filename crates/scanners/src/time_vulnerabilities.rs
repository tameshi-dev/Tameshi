...
use thalir_core::{
    ...
    contract::Contract,
    instructions::{Instruction, ContextVariable},
};
use anyhow::Result;
...
impl IRTimeVulnerabilityScanner {
    ...
    pub fn analyze(&mut self, contract: &Contract) -> Result<Vec<Finding>> {
        ...
        for (func_name, function) in &contract.functions {
                ...
                for (idx, instruction) in block.instructions.iter().enumerate() {
                    match instruction {
                        Instruction::GetContext { result, var } => {
                            match var {
                                ...
                                ContextVariable::BlockDifficulty => {
                                    block_dependencies.push((block_id, idx, result, "block.difficulty"));
                                }
                                _ => {}
                            }
                        }
                        ...
                        Instruction::Require { condition, message } => {
                            if self.uses_time_value(condition, &time_dependencies, &block_dependencies) {
                                control_flow_usage.insert(
                                    (block_id, idx), 
                                    ("require", message.as_str())
                                );
                            }
                        }
                        ...
                        Instruction::Div { result, left, right, .. } => {
                            if self.value_depends_on_time(left, &time_dependencies, &block_dependencies) ||
                               self.value_depends_on_time(right, &time_dependencies, &block_dependencies) {
            ...
            self.analyze_timestamp_dependence(contract, func_name, &time_dependencies, &control_flow_usage);
            self.analyze_block_manipulation(contract, func_name, &block_dependencies, &control_flow_usage);
            self.analyze_time_arithmetic(contract, func_name, &arithmetic_usage, &time_dependencies);
    ...
    fn uses_time_value(
        ...
        time_deps: &[(thalir_core::block::BlockId, usize, &thalir_core::values::Value, &str)],
        block_deps: &[(thalir_core::block::BlockId, usize, &thalir_core::values::Value, &str)]
    ) -> bool {
    ...
    fn analyze_timestamp_dependence(
        ...
        contract: &Contract,
        func_name: &str,
        time_dependencies: &[(thalir_core::block::BlockId, usize, &thalir_core::values::Value, &str)],
    ...
    fn analyze_block_manipulation(
        ...
        contract: &Contract,
        func_name: &str,
        block_dependencies: &[(thalir_core::block::BlockId, usize, &thalir_core::values::Value, &str)],
    ...
    fn analyze_time_arithmetic(
        ...
        contract: &Contract,
        func_name: &str,
        arithmetic_usage: &HashMap<(thalir_core::block::BlockId, usize), &thalir_core::values::Value>,
        time_dependencies: &[(thalir_core::block::BlockId, usize, &thalir_core::values::Value, &str)]
    ) {
